...
def formatar_como_tabela(dados):
    ...
    print("=" * 60)
    print(f"{'Estado':<10} {'Transição 0':<15} {'Transição 1':<15}")
    print("-" * 60)
...
while i < len(transicoes_AFD):
    ...
    for j in estado_quebrado:
        ...
        transicoes_AFD[i][1][0] = "".join(sorted(transicoes_AFD[i][1][0]))
        transicoes_AFD[i][1][1] = "".join(sorted(transicoes_AFD[i][1][1]))
    ...
    while cont < len(transicoes_AFD):
        ...
        for conexao in conexoes:
            ...
            if conexao == "" or conexao is None:
                continue
            # Verifica se o elemento já existe em alguma lista[i][0]
            existe = any(conexao == item[0] for item in transicoes_AFD)

            # Se não existe, adiciona um novo item com sublista vazia
            if not existe:
